...
# then log output of file /code/bin/abhihg
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/abhihg \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/abhihg | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/abhihg /usr/bin/abhihg
