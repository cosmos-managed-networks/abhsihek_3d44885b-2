...
HTTPS_GIT := github.com/abhi/abhsihek.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=abhsihek \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=abhihg \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/abhihg ./cmd/abhihg
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/abhihg.exe ./cmd/abhihg
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/abhihg
...
draw-deps:
	...
	@goviz -i ./cmd/abhihg -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c abhsihek --local -f chains.yaml
endif
...
set-testnet-configs:
	abhihg config set client chain-id chainid-1
	abhihg config set client keyring-backend test
	abhihg config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | abhihg keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | abhihg keys add acc1 --recover`
